...
function showTopicDetail(t){const ev=arr(t.evidence);detail.innerHTML=`<div class="eyebrow">${esc(t.community)} · 话题分析</div><h2>${esc(t.label_zh||'未命名话题')}</h2><div class="en">${esc(t.label_en||'')}</div><div class="score">${esc(t.heat_score||0)}<small>/100</small></div><div class="tags"><span class="tag ${t.status==='formal'?'formal':'weak'}">${t.status==='formal'?'正式话题':'弱信号'}</span><span class="tag">趋势：${esc(t.trend||'unknown')}</span><span class="tag">${esc(t.post_count||0)} 帖 · ${esc(t.author_count||0)} 作者 · ${esc(t.commenter_count||0)} 评论者</span></div><div class="summary">${esc(t.summary||'未知')}</div>${field('涉及车型 / 平台 / 场景',[...arr(t.vehicles),...arr(t.platforms),...arr(t.scenarios)])}${field('用户痛点',t.pains)}${field('用户需求',t.needs)}${field('当前方案',t.current_solutions)}${field('方案不足',t.gaps)}${field('机会假设（不是开品结论）',t.opportunity_hypotheses)}${field('下一步验证问题',t.validation_questions)}<div class="evidence"><h3>帖子 / 评论证据（${ev.length}）</h3>${ev.length?ev.map(e=>`<div class="ev"><a href="${esc(e.url||'#')}" target="_blank" rel="noreferrer">打开 Reddit 原文 ↗</a><div class="claim">${esc(e.claim_en||'')}</div><div class="zh">${esc(e.claim_zh||'')}</div></div>`).join(''):'<div class="empty">暂无可回溯证据</div>'}</div>`;}
